...
def load_boxes_and_labels(gt_box_list, exclusion_file, is_train=False, full_test_on_val=False):
    ...
    Returns:
        all_boxes (dict): a dict which maps from `video_name` and
            ...
            labels for the box.
        all_boxes[video_name][frame_sec][`box_coord`, `box_labels`]
        {'<video_name>': {<frame_num>: a list of [box_i, box_i_labels]} }
    """
...
def get_keyframe_data(boxes_and_labels):
    ...
    def sec_to_frame(sec):
        ...
        if sec == 1 and sec == 2:
            sec01 = 30
        if 3<= sec <= 31:
            sec01 = int((sec-1.5)*FPS)
        if sec == 32:                    #原方法 (sec - 0) * FPS
            sec01 = 900
        return sec01
